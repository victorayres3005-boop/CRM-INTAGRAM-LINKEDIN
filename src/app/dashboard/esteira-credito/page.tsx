...
const FASES_APROVADO  = new Set(["Crédito Aprovado"]);
const FASES_NEGADO    = new Set(["Crédito Negado"]);
const FASES_COMITE    = new Set(["Comitê de Crédito", "Questionamentos comitê"]);
const FASES_JURIDICO  = new Set(["Análise Jurídica"]);

const ORDEM_FASE: Record<string, number> = {
  "Aguardando Análise":       0,
  "Análise de Crédito":       1,
  "Esclarecimentos Comerciais": 2,
  "Comitê de Crédito":        3,
  "Questionamentos comitê":   4,
  "Análise Jurídica":         5,
  "Crédito Aprovado":         6,
  "Crédito Negado":           7,
};
...
function faseColor(fase: string): string {
  ...
  if (FASES_JURIDICO.has(fase))  return "bg-teal-50 text-teal-700 border-teal-200";
  if (fase === "Esclarecimentos Comerciais") return "bg-amber-50 text-amber-700 border-amber-200";
...
function faseFill(fase: string): string {
  ...
  if (FASES_JURIDICO.has(fase))  return "#0d9488";
  if (fase === "Esclarecimentos Comerciais") return "#d97706";
  return "#203b88";
}
...
function CardModal({ card, onClose }: { card: CardEsteira; onClose: () => void }) {
  useEffect(() => {
        ...
        <div className="bg-gradient-to-br from-cf-navy to-cf-navy/80 px-6 py-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-white/50 uppercase tracking-wider mb-1">Esteira de Crédito</p>
...
export default function EsteiraCreditoPage() {
  const [esteira, setEsteira] = useState<CardEsteira[]>([]);
  ...
  const [cardSelecionado, setCardSelecionado] = useState<CardEsteira | null>(null);
  ...
  async function carregar(isAuto = false) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/esteira-credito", { cache: "no-store" });
      if (!res.ok) {
  ...
  const fasesUnicas    = ["Todos", ...Array.from(new Set(esteira.map((c) => c.fase).filter(Boolean))).sort(
    (a, b) => (ORDEM_FASE[a] ?? 99) - (ORDEM_FASE[b] ?? 99)
  )];
  const gerentesUnicos = ["Todos", ...Array.from(new Set(esteira.map((c) => c.gerente).filter((g) => g !== "—"))).sort()];
  const mesesUnicos    = ["Todos", ...Array.from(new Set(esteira.map((c) => getMes(c.dataEntrada)).filter(Boolean) as string[])).sort()];

  const filtrados = esteira.filter((c) => {
  ...
  const aprovados = filtrados.filter((c) => FASES_APROVADO.has(c.fase)).length;
  const negados   = filtrados.filter((c) => FASES_NEGADO.has(c.fase)).length;
  const comite    = filtrados.filter((c) => FASES_COMITE.has(c.fase)).length;
  const emAnalise = total - aprovados - negados - comite;
  ...
  const porMes = mesesUnicos
    .filter((m) => m !== "Todos")
    .map((mes) => {
      const list = esteira.filter((c) => getMes(c.dataEntrada) === mes);
      return {
        mes: mesLabel(mes),
        total: list.length,
        aprovados: list.filter((c) => FASES_APROVADO.has(c.fase)).length,
        negados: list.filter((c) => FASES_NEGADO.has(c.fase)).length,
      };
    });
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold text-cf-text1">Esteira de Crédito</h2>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "Total na Esteira", value: total,     icon: Layers,       color: "navy"    as const },
            { label: "Crédito Aprovado", value: aprovados, icon: CheckCircle2, color: "green"   as const },
            { label: "Em Comitê",        value: comite,    icon: Users2,       color: "violet"  as const },
            { label: "Crédito Negado",   value: negados,   icon: XCircle,      color: "danger"  as const },
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
          <div className="cf-card p-6">
            ...
            <ResponsiveContainer width="100%" height={Math.max(200, porFase.length * 34)}>
              <BarChart data={porFase} layout="vertical" barSize={14}>
                ...
                <YAxis dataKey="fase" type="category" tick={{ fontSize: 10, fill: "#374151" }} width={155} axisLine={false} tickLine={false} />
          ...
          <div className="cf-card p-6">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={porMes} barSize={20}>
                ...
                <Tooltip content={<BrandTooltip />} />
                <Bar dataKey="total"     fill="url(#gEsteiraTotal)" radius={[5, 5, 0, 0]} name="Total" />
                <Bar dataKey="aprovados" fill="url(#gEsteiraAprov)" radius={[5, 5, 0, 0]} name="Aprovados" />
                <Bar dataKey="negados"   fill="url(#gEsteiraNeg)"   radius={[5, 5, 0, 0]} name="Negados" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {filtrados.map((c) => (
                  ...
                  >
                    <td className="px-6 py-3 font-medium text-cf-text1 max-w-[220px] truncate">{c.razaoSocial}</td>
                    <td className="px-4 py-3 text-cf-text3 font-mono">{c.cnpj || "—"}</td>
